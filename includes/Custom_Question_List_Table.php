...
class Custom_Question_List_Table extends WP_List_Table
{
  private $post_data;
  ...
  function prepare_items()
  {
    ...
    $this->items = $wpdb->get_results($wpdb->prepare(
      ...
      WHERE q.correct_msg = '' AND q.quiz_id IN (" . implode(',', array_map('intval', $selected_quiz['quizzes'])) . ")
      LIMIT %d OFFSET %d",
      $per_page,
...
function custom_question_plugin_page()
{

?>
  <div class="wrap">
    <h1>Questions List</h1>
    <?php render_question_form('custom-question-plugin');
    if (isset($_GET['quizzes'])) {
      $question_list_table = new Custom_Question_List_Table($_GET);
      $question_list_table->prepare_items();
    ?>
      <form method="post">
        <input type="hidden" name="page" value="custom-question-plugin" />
...
<?php
  ...
  add_action('wp_ajax_delete_question', 'delete_question_callback');
